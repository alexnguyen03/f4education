...
import {Breadcrumbs, Anchor, Text, Loader, Button, Image, Card, Box, Flex, Group, Rating, Skeleton, Checkbox, HoverCard, Grid} from '@mantine/core';
...
function Cart() {
	...
	useEffect(() => {
		...
		let newTotalPrice = 0;
		carts.length > 0 && carts.map((item) => (newTotalPrice += item.course.coursePrice));
	...
	return (
		<>
						...
						<>
							<Grid>
								<Grid.Col
									...
									md='12'
									sm='12'>
									<h5 className='font-weight-600 text-dark'>{carts.length} khóa học trong giỏ hàng</h5>
														...
														<Grid>
															<Grid.Col
																...
																<Link
																	...
																	to={`/course/${cart.course.courseId}`}
																	key={index}>
																	<p className='font-weight-700 text-dark m-0 p-0'>{cart.course.courseName}</p>
																	<span className='text-muted'>{/* Môn học: <strong>{cart.subjectName}</strong> */}</span>
																	<div className='d-flex text-dark'>
																		<span className='font-weight-600'>{/* {cart.ratings} */}4.6</span>
																		<div className='mx-2'>
																		...
																		<span className='text-muted'>{/* ({cart.reviews}) */} (39.930)</span>
																	</div>
																	<div className='d-flex justify-content-start'>
																		<span className='text-muted'>{cart.course.courseDuration}</span>
																		<span className='mx-2'>-</span>
																		<span className='text-muted'>{cart.course.numberSession} bài giảng</span>
																		<span className='mx-2'>-</span>
			...
			{/* Newest Course */}
			<h3 className='font-weight-700 text-dark my-5'>Những khóa học mới nhất</h3>
			{/* Mantine Carousel */}
			<Carousel
				slideSize='25%'
				height='350px'
